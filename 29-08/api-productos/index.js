...
app.get('/productos', (req, res) => {
    ...
    if (precioMin) {
        if (isNaN(precioMin)) {
            return res.status(400).json({ success: false, message: 'El precio mínimo debe ser un número' })
        }
    ...
    if (precioMax) {
        if (isNaN(precioMax)) {
            return res.status(400).json({ success: false, message: 'El precio máximo debe ser un número' })
        }
    ...
    if (categoria) {
        ...
        productosFiltrados = productosFiltrados.filter(p => p.categoria.toLowerCase().includes(categoria.toLowerCase()))
    }
    ...
    if (cantidad) {
        ...
        productosFiltrados = productosFiltrados.filter(p => p.cantidad >= cantidad)
    }
...
app.post('/productos', (req, res) => {
    ...
    if (!nombre || !categoria || cantidad === undefined || precio === undefined) {
        return res.status(400).json({ success: false, message: 'Todos los campos (nombre, categoria, cantidad, precio) son obligatorios' })
    }
...
app.put('/productos/:id', (req, res) => {
    ...
    // Validar que todos los campos estén presentes
    if (nombre === undefined || categoria === undefined || cantidad === undefined || precio === undefined) {
        return res.status(400).json({ success: false, message: 'Todos los campos (nombre, categoria, cantidad, precio) son obligatorios' })
    }

    // Validar nombre
    if (nombre.trim() === '' || nombre.trim().length > 100) {
        return res.status(400).json({ success: false, message: 'El campo nombre esta vacío o excede los 100 caracteres' })
    }

    // Validar categoría
    if (categoria.trim() === '') {
        return res.status(400).json({ success: false, message: 'El campo categoria esta vacío' })
    }
